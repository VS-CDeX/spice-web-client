...
wdi.ClientGui = $.spcExtend(wdi.EventObject.prototype, {
  ...
  init: function init(c) {
    ...
    this.stuckKeysHandler.addListener('inputStuck', this._sendInput.bind(this), this); //load magnifier background

    this.magnifierBackground = window.$('<img/>');
    this.magnifierBackground.attr('src', 'assets/resources/magnifier.png');
    this.initSound();
    this.inputManager = c.inputManager || new wdi.InputManager({
  ...
  _sendInput: function _sendInput(params) {
    var data = params;
    var type = data[0];
    var event = data[1];
  ...
  getContext: function getContext(surface_id) {
    return this.contexts[surface_id];
  },
  getCanvas: function getCanvas(surface_id) {
    return this.canvas[surface_id];
  },
  checkFeatures: function checkFeatures() {
  ...
  deleteSubCanvas: function deleteSubCanvas(window) {
    var obj = this.subCanvas[window['hwnd']];
    this.subCanvas[window['hwnd']] = null;
    return obj;
  },
  moveSubCanvas: function moveSubCanvas(window) {
    var obj = this.subCanvas[window['hwnd']];
    obj['info'] = window;
  ...
  resizeSubCanvas: function resizeSubCanvas(window) {
    var obj = this.subCanvas[window['hwnd']];
  ...
  _fillSubCanvasFromWindow: function _fillSubCanvasFromWindow(window) {
  ...
  createNewSubCanvas: function createNewSubCanvas(window) {
  ...
  fillSubCanvas: function fillSubCanvas(filterPosition) {
  ...
  _doDrawSubCanvas: function _doDrawSubCanvas(canvas, subCanvas, info) {
  ...
  removeCanvas: function removeCanvas(spiceMessage) {
  ...
  drawCanvas: function drawCanvas(spiceMessage) {
    var surface = spiceMessage.args;
  ...
  disableKeyboard: function disableKeyboard() {
  ...
  setCanvasMargin: function setCanvasMargin(canvasMargin) {
  ...
  createEventLayer: function createEventLayer(event_id, width, height) {
    var self = this;
    ...
    eventLayer.bind('touchstart', function (event) {
      ...
      if (self.mouse_mode == wdi.SpiceMouseModeTypes.SPICE_MOUSE_MODE_CLIENT) {
        ...
        self.generateEvent.call(self, 'mousemove', [x - offset.left - wdi.VirtualMouse.lastMousePosition.x, y - offset.top - wdi.VirtualMouse.lastMousePosition.y, self.mouse_status, self.mouse_mode]);
      }
    ...
    eventLayer.bind('touchmove', function (event) {
      var touch = event.originalEvent.touches[0] || event.originalEvent.changedTouches[0];
      var x = touch.pageX;
      var y = touch.pageY; //TODO: ignore first move
      ...
      if (event.originalEvent.touches.length === 1 && self.enabledTouchMove) {
        ...
        if (self.mouse_mode == wdi.SpiceMouseModeTypes.SPICE_MOUSE_MODE_CLIENT) {
          ...
          self.generateEvent.call(self, 'mousemove', [x - offset.left - wdi.VirtualMouse.lastMousePosition.x, y - offset.top - 80 - wdi.VirtualMouse.lastMousePosition.y, self.mouse_status, self.mouse_mode]);
        }

        var myX = x - offset.left;
        var myY = y - offset.top; //draw magnifier
        ...
        ctx.clip();
        ctx.drawImage(self.getCanvas(0), myX - 50, //-50 because we are going to get
        ...
        //is working
        100, 100, 0, 0, 150, 150); //				//draw the background

        ctx.drawImage(self.magnifierBackground[0], 0, 0);
        ctx.restore(); //empty magnifier
      } else if (event.originalEvent.touches.length === 2) {
    ...
    eventLayer['mousemove'](function (event) {
      ...
      if (self.mouse_mode == wdi.SpiceMouseModeTypes.SPICE_MOUSE_MODE_CLIENT) {
        ...
        var dy = e.movementY || e.mozMovementY || e.webkitMovementY || 0; // Sometimes mousemove events with dx == dy == 0 are generated.
        // For instance mouse click in chrome/windows.

        if (!dx && !dy && typeof e.movementX == 'undefined' && typeof e.mozMovementX == 'undefined' && typeof e.webkitMovementY == 'undefined') {
  ...
  launchRightClick: function launchRightClick(x, y) {
    var self = this;
  ...
  launchMouseDown: function launchMouseDown(x, y) {
    var self = this;
  ...
  generateEvent: function generateEvent(event, params) {
  ...
  motion_ack: function motion_ack() {
    this.ack_wait = 0;
  },
  setMouseMode: function setMouseMode(mode) {
  ...
  updateMousePointer: function updateMousePointer() {
  ...
  handleKey: function handleKey(e) {
    ...
    if (e.ctrlKey && !e.altKey || wdi.Keymap.isInKeymap(e.keyCode) && e.type !== "keypress") {
      e.preventDefault();
  ...
  initSound: function initSound() {
    var self = this; //		if (!Modernizr.touch) {
  ...
  capturePointer: function capturePointer() {
    if (!document.pointerLockElement && !document.mozPointerLockElement && typeof this.eventLayer.requestPointerLock === "function") {
